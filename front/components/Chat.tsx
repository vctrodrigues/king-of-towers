...
const sendMessage = (message : any) =>{
  if(message){
    const date = new Date();
    const newMessage : Message = {text: message, user: user, date: date.toLocaleTimeString()}
  ...
  return (
    ...
    >
      <Flex direction="column" gap="1" className="max-h-full overflow-auto" width='400px'>
      ...
      <Flex direction="row" className="w-full h-fit">
        <TextField.Root onChange={(param) => setMessage(param.target?.value)} id={"input-text"} onKeyDown={(e) => e.key === 'Enter' ? sendMessage(message) : ''}>
          <TextField.Slot></TextField.Slot>
        </TextField.Root>
        <Button onClick={() => sendMessage(message)} style={{marginLeft: '10px'}}>Enviar</Button>
      </Flex>
    </Flex>
  );
};
